...
There are two ways to use this, depending on whether you use an AI agent.

---

### 🤖 Option A — Via Agent (Hermes or OpenClaw) [Recommended]

This is the easiest path. Your agent reads the SKILL file, understands all the
instructions, and sets everything up automatically.

**Step 1.** Clone or download this repo.

**Step 2.** Set your NVIDIA API Key (get one free at [build.nvidia.com](https://build.nvidia.com)):
...
**Step 3.** Ask your agent to install the skill:

```
# Example prompt to your agent:
"Please install the rate limiter skill from SKILL_Hermes_RateLimiterNIM.md"
```

The agent will read the SKILL file, create the `scripts/` folder, write
`rate_limiter_nim.py`, and verify the installation. No manual steps needed.

---

### 🛠️ Option B — Manual (no agent)
...
**Step 2.** Set your NVIDIA API Key (see above).

**Step 3.** Open `SKILL_Hermes_RateLimiterNIM.md` or `SKILL_OpenClaw_RateLimiterNIM.md`,
copy the Python code block from the **"Full Python Module"** section,
and save it as `rate_limiter_nim.py` next to your script.
...
# The rate limiter handles all waits and retries automatically.
...
├── README.md                          ← This file
│
├── RateLimiter_NIM_Guia_Universal.md  ← Full technical guide (EN)
│
├── SKILL_Hermes_RateLimiterNIM.md     ← Skill file for Hermes agent
│
└── SKILL_OpenClaw_RateLimiterNIM.md   ← Skill file for OpenClaw agent
```

> **Note:** There is no `rate_limiter_nim.py` in the repo intentionally.
> The SKILL files contain the full Python code and instruct the agent to create
> the file automatically during installation. The agent does the work for you.
...
The SKILL files are self-contained: they include all instructions, the full Python
code, and setup commands. Just hand the file to your agent and it will do the rest.
...
1. Create `~/.hermes/skills/mlops-inference/rate-limiter-nim/scripts/`
2. Write `rate_limiter_nim.py` from the code in the SKILL file
3. Install `requests` if not already present
4. Read `NVIDIA_API_KEY` from `~/.hermes/.env` (no extra config needed)

**To trigger it**, copy the SKILL file to your Hermes skills folder:

```bash
mkdir -p ~/.hermes/skills/mlops-inference/rate-limiter-nim
cp SKILL_Hermes_RateLimiterNIM.md ~/.hermes/skills/mlops-inference/rate-limiter-nim/SKILL.md
```

Then ask Hermes: *"Set up the rate-limiter-nim skill"*
...
**What the agent will do automatically:**
1. Create `~/.openclaw/skills/rate-limiter-nim/scripts/`
2. Write `rate_limiter_nim.py` from the code in the SKILL file
3. Install `requests` if not already present
4. Read `NVIDIA_API_KEY` from the environment

**To trigger it**, copy the SKILL file to your OpenClaw skills folder:

```bash
mkdir -p ~/.openclaw/skills/rate-limiter-nim
cp SKILL_OpenClaw_RateLimiterNIM.md ~/.openclaw/skills/rate-limiter-nim/SKILL.md
```

Then ask OpenClaw: *"Set up the rate-limiter-nim skill"*
